...
//        commandManager.addCommand(new SaveCommand(this, collectionManager));
    ...
    public void lockable(boolean lock) {
        this.lock = lock;
    }
    ...
    public void run() {
        ...
        while (state == State.RUNNING) {
            try {
                if (!collectionManager.isInit()) {
                    actionManager.addFirst(collectionManager::init);
                }
                if (actionManager.size() == 0) actionManager.add(
                        () -> {
                ...
                if (ScriptInputStrategy.scriptStackSize() > 0) inputManager.setStrategy(ScriptInputStrategy.pop());
                else {
                    consoleMode();
                    ConsolePrinter.printError(e.getMessage());
                }
